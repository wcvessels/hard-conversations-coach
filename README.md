...
- **Static and behavioral verification.** Pre-submission static audits live in the source workspace's `audits/` folder (not published here per scope discipline). Behavioral testing was run in a real Claude Sonnet 4.6 project session against the highest-risk gates in the assessor protocol — strong pass evidence on R6 hard-stop (Prompt 1, including a notable uncovered-risk discovery where the coach surfaced a position-elimination/performance-rationale contradiction the manager hadn't named), vagueness-gap block (Prompts 2 & 3), hearsay flag (Prompt 3), and listicle refusal (Prompt 4). Prompt 5 (skip-roleplay bargain) was not independently re-run in this round; coverage is sufficient for high-risk-gate confirmation, and judges are encouraged to re-run the full protocol in a fresh project per [`ASSESSOR_GUIDE.md`](ASSESSOR_GUIDE.md).
